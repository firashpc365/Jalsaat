...
type AnalyzedService = ServiceItem & {
    profit: number;
    profitMargin: number;
};
    ...
    const analyzedData = useMemo((): AnalyzedService[] => {
        return services
            ...
            .map(s => {
                ...
                // Use defined cost if available, otherwise estimate
                const cost = s.defaultCost || (basePrice * 0.6); 
                        ...
                        <tbody className="divide-y" style={{ borderColor: 'var(--border-color)'}}>
                            {analyzedData.map(item => (
                                <tr key={item.id} className="hover:bg-black/10">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-red-400">{(item.defaultCost || ((item.basePrice || 0) * 0.6)).toLocaleString()}</td>
