...
import { MenuIcon, SparkleIcon, PlusIcon, EditIcon, TrashIcon, DocumentTextIcon, ChartBarIcon, BoltIcon, CalendarIcon, MapPinIcon, UsersIcon, CreditCardIcon, EyeIcon, DownloadIcon, XIcon, CheckCircleIcon, LinkIcon } from './common/icons';
...
export const EventDetail: React.FC<EventDetailProps> = ({
    ...
    const handleSaveDetails = () => {
        // Validation handled implicit via form controls, specific client linking logic below
    ...
    const handleClientChange = (clientId: string) => {
        ...
        if (client) {
            setEditedEvent(prev => ({
                ...
                clientContact: client.primaryContactName
            }));
        }
    };
    ...
    return (
        <div className="h-full overflow-y-auto custom-scrollbar p-4 sm:p-6 space-y-6 pb-20">
            {/* Header */}
                ...
                <div className="flex items-center gap-2 w-full xl:w-auto">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                            <h1 className="text-2xl sm:text-3xl font-extrabold break-words leading-tight" style={{ color: 'var(--text-primary-color)' }}>{event.name}</h1>
                            <span className={`flex-shrink-0 px-2.5 py-1 text-xs font-bold uppercase rounded border w-fit ${statusColors[event.status] || 'bg-gray-700 text-gray-300 border-gray-600'}`}>
                                {event.status}
                            </span>
                        </div>
                        <p className="text-sm opacity-70 flex flex-wrap items-center gap-2 mt-1" style={{ color: 'var(--text-secondary-color)' }}>
                             ...
                             {event.eventType && <><span className="hidden sm:inline">•</span> <span className="text-[var(--primary-accent-color)] font-medium">{event.eventType}</span></>}
                             {event.clientId && <><span className="hidden sm:inline">•</span> <span className="flex items-center gap-1 text-blue-400"><LinkIcon className="h-3 w-3"/> Linked</span></>}
                        </p>
                    </div>
                </div>
            ...
            {/* ... Rest of AI Toolbar and Body same as before ... */}
            
            {/* AI Toolbar */}
            <div className="p-4 rounded-xl border bg-gradient-to-r from-purple-900/20 to-blue-900/20 border-white/10 overflow-x-auto custom-scrollbar shrink-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {hasTheme && (
                    <div className="lg:col-span-3 relative overflow-hidden rounded-2xl border group shadow-xl shrink-0" style={{ backgroundColor: 'var(--card-container-color)', borderColor: 'var(--border-color)' }}>
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-6 rounded-xl border shadow-lg" style={{ backgroundColor: 'var(--card-container-color)', borderColor: 'var(--border-color)' }}>
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            {event.clientId && (
                                <div className="mt-2 text-xs text-blue-400 flex items-center justify-end gap-1">
                                    <LinkIcon className="h-3 w-3" /> Linked Record
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        <ul className="space-y-2 overflow-y-auto custom-scrollbar flex-grow pr-1 max-h-[300px]">
                            {sortedTasks.map(task => {
                                ...
                                return (
                                    <li 
                                        key={task.id} 
                                        className={`flex items-start gap-3 p-3 rounded-lg border transition-all duration-500 group ${
                                    ...
                                    >
                                        <div className="pt-0.5 flex-shrink-0 relative flex items-center justify-center w-5 h-5">
                                            <input 
                                                type="checkbox" 
                                                checked={task.isCompleted} 
                                                onChange={() => handleToggleTask(task.id)}
                                                className={`appearance-none w-5 h-5 border-2 rounded-md cursor-pointer transition-all duration-300 ease-out focus:outline-none ${
                                                    task.isCompleted 
                                                        ? 'bg-green-500 border-green-500 scale-100 shadow-[0_0_10px_rgba(34,197,94,0.4)]' 
                                                        : status === 'overdue' 
                                                            ? 'border-red-400 hover:border-red-300 bg-transparent'
                                                            : 'border-slate-500 hover:border-[var(--primary-accent-color)] bg-transparent'
                                                }`}
                                            />
                                            <svg 
                                                className={`absolute w-3.5 h-3.5 text-white pointer-events-none transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                                                    task.isCompleted ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-50 -rotate-90'
                                                }`} 
                                                fill="none" 
                                                viewBox="0 0 24 24" 
                                                stroke="currentColor" 
                                        ...
                                        <div className="flex-grow min-w-0">
                                            <p className={`text-sm leading-snug transition-all duration-300 break-words ${task.isCompleted ? 'line-through text-slate-500 opacity-60' : 'text-[var(--text-primary-color)]'}`}>
                                                {task.description}
                                            </p>
                                            {task.dueDate && (
                                                <div className={`text-[10px] mt-1.5 flex items-center gap-1.5 font-medium transition-opacity duration-300 ${task.isCompleted ? 'opacity-50' : 'opacity-100'} ${
                                                    status === 'overdue' ? 'text-red-400' : 
                                                    status === 'due-today' ? 'text-amber-400' : 
                                                    task.isCompleted ? 'text-slate-600' : 'text-slate-400'
                                                }`}>
                                                    {(status === 'overdue' || status === 'due-today') && !task.isCompleted && <BoltIcon className="h-3 w-3 animate-pulse" />}
                                                    <span>
                                        ...
                                        <button 
                                            ...
                                            className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-400 p-1 transition-all transform hover:scale-110 hover:bg-white/5 rounded"
                                            title="Delete Task"
                                        >
                ...
                <Modal title="Edit Event Details" onClose={() => setIsEditingDetails(false)} onSave={handleSaveDetails}>
                    <div className="space-y-4">
                        ...
                        <div className="bg-black/20 p-4 rounded-xl border border-white/5">
                             <label className="block text-sm font-medium mb-1 text-[var(--text-secondary-color)]">Linked Client</label>
                             <select 
                                value={editedEvent.clientId || ''} 
                                onChange={(e) => handleClientChange(e.target.value)}
                                className="w-full p-2 border rounded shadow-sm bg-black/10 focus:border-[var(--primary-accent-color)] outline-none text-[var(--text-primary-color)]"
                             ...
                             <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                                <div className="p-2 bg-white/5 rounded">
                                    <span className="opacity-50 block">Contact Name</span>
                                    <span className="font-bold">{editedEvent.clientContact}</span>
                                </div>
                             </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                             <div>
                                ...
                                <select
                                    value={editedEvent.eventType || 'Other'}
