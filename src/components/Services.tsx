...
import { PlusIcon, MenuIcon, EditIcon, Squares2X2Icon, ListBulletIcon, CubeTransparentIcon, TrashIcon, SparkleIcon, DocumentTextIcon, EyeIcon } from './common/icons';
import { ConfirmationModal } from './common/ConfirmationModal';
import { getAIServiceAssistance } from '../services/geminiService';
...
interface ServicesProps {
  ...
  onBulkUpdateServices: (updates: {id: string, data: Partial<ServiceItem>}[]) => void;
  onMenuClick: () => void;
...
const ServiceCard: React.FC<{ item: ServiceItem; onClick: () => void; onEdit: () => void; onDelete: () => void; canManage: boolean }> = ({ item, onClick, onEdit, onDelete, canManage }) => (
  <div 
    onClick={onClick}
    className="backdrop-blur-xl border shadow-lg p-4 flex flex-col justify-between h-full group relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl cursor-pointer"
    style={{
    ...
    <div>
      <div className="relative overflow-hidden rounded-lg mb-4 aspect-video">
        ...
        <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
             <div className="p-1.5 bg-black/60 backdrop-blur-sm rounded-full text-white hover:bg-blue-500 transition shadow-lg" title="View Details">
      ...
      <div className="flex justify-between items-start mb-1">
        <h3 className="text-lg font-bold leading-tight line-clamp-1" style={{ color: 'var(--text-primary-color)'}}>{item.name}</h3>
      </div>
...
const ServiceListItem: React.FC<{ item: ServiceItem; onClick: () => void; onEdit: () => void; onDelete: () => void; canManage: boolean }> = ({ item, onClick, onEdit, onDelete, canManage }) => (
    <div 
        onClick={onClick}
        className="flex items-center p-3 border-b hover:bg-black/10 group transition-colors cursor-pointer"
    ...
    >
        ...
        <div className="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
            ...
            <div className="col-span-2 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                {canManage && (
                    <>
                        <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-1.5 hover:bg-white/10 rounded text-[var(--text-secondary-color)] hover:text-[var(--primary-accent-color)]" title="Edit">
                            <EditIcon className="h-4 w-4" />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-1.5 hover:bg-red-500/10 rounded text-red-400 hover:text-red-300" title="Delete">
                            <TrashIcon className="h-4 w-4" />
                        </button>
                    </>
                )}
...
export const Services: React.FC<ServicesProps> = ({ services, user, setError, setSuccess, onAddItem, onUpdateService, onDeleteService, onBulkAddServices, onBulkUpdateServices, onMenuClick, onLogAIInteraction, settings, onAIFallback }) => {
  const [isEditorOpen, setIsEditorOpen] = useState(false);
  const [editingService, setEditingService] = useState<ServiceItem | null>(null);
  const [viewingService, setViewingService] = useState<ServiceItem | null>(null); // State for detail modal
  const [isBulkCreatorOpen, setIsBulkCreatorOpen] = useState(false);
  const [isCatalogueStudioOpen, setIsCatalogueStudioOpen] = useState(false);
  ...
  const handleAutoFillFeatures = async () => {
      const servicesToUpdate = services.filter(s => !s.keyFeatures || s.keyFeatures.length === 0);
      
      if (servicesToUpdate.length === 0) {
          setSuccess("All services already have features.");
          return;
      }

      setIsAutoFilling(true);
      const uniqueCategories: string[] = Array.from(new Set(services.map(s => s.category)));
      
      try {
          // Process in chunks of 3 to avoid rate limits
          const chunkSize = 3;
          const updates: {id: string, data: Partial<ServiceItem>}[] = [];
          ...
          for (let i = 0; i < servicesToUpdate.length; i += chunkSize) {
              ...
              const chunkPromises = chunk.map(async (service) => {
                  try {
                      // Requesting 'features' assistance with explicit instruction for 3 items
                      const { result, fullPrompt } = await getAIServiceAssistance('features', service, settings, uniqueCategories);
                      
                      onLogAIInteraction({
                          feature: 'service_features',
                          promptSummary: `Auto-fill features for ${service.name}`,
                          fullPrompt,
                          response: JSON.stringify(result)
                      });

                      const features = Array.isArray(result) ? (result as any[] as string[]) : (result as string).split('\n').filter(x => x.trim());
                      return { id: service.id, data: { keyFeatures: features } };
                  } catch (e) {
                      console.error(`Failed features for ${service.name}`, e);
                      return null;
                  }
              });
              
              const chunkResults = await Promise.all(chunkPromises);
              updates.push(...chunkResults.filter((r): r is {id: string, data: Partial<ServiceItem>} => r !== null));
          }

          if (updates.length > 0) {
              onBulkUpdateServices(updates);
              setSuccess(`Auto-filled features for ${updates.length} services.`);
          }
      } catch (err) {
          setError(err);
      } finally {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 h-full flex flex-col relative overflow-hidden min-h-screen">
      ...
      <div className="flex justify-between items-center flex-wrap gap-y-2 shrink-0">
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 ml-2 border-l pl-4 border-white/10">
                 <button
                    ...
                    className="px-3 py-2 bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 font-semibold text-sm shadow-md hover:bg-indigo-600/30 transition-all duration-300 flex items-center rounded-lg disabled:opacity-50"
                    title="Auto-generate missing features"
                >
                    {isAutoFilling ? <InlineSpinner /> : <SparkleIcon className="h-5 w-5 mr-2" />}
                    <span className="hidden sm:inline">Auto-Fill</span> Features
                </button>
                 <button
      ...
      {displayMode === 'catalogue' ? (
        ...
        <>
            {/* Admin View Toolbar */}
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 p-3 rounded-xl border shadow-sm animate-in-item shrink-0" style={{ backgroundColor: 'var(--card-container-color)', borderColor: 'var(--border-color)' }}>
                        ...
                        {processedServices.map((item, index) => (
                            <div key={item.id} className="animate-in-item" style={{ animationDelay: `${index * 30}ms` }}>
                                <ServiceCard 
                                    ...
                                    onDelete={() => setServiceToDelete(item.id)} 
                                    canManage={canManage} 
                                />
                            </div>
                        ))}
