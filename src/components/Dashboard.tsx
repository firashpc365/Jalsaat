...
import { PlusIcon, SparkleIcon, MenuIcon, TrendingUpIcon, CashIcon, DocumentTextIcon, ImageIcon, ExpandIcon, CollapseIcon, SettingsIcon, ChartBarIcon, BoltIcon, InformationCircleIcon, Squares2X2Icon } from './common/icons';
...
const ExpandableInsightCard: React.FC<ExpandableInsightCardProps> = ({ title, value, icon, note, data, reportType, aiContextData, onLogInteraction, tooltip }) => {
                ...
                <div className="flex gap-2">
                    {tooltip && (
                        <div className="group/tooltip relative" onClick={(e) => e.stopPropagation()}>
                            <div className="p-1.5 rounded-full text-slate-400 hover:text-white cursor-help transition-colors bg-white/5 hover:bg-white/10">
                                <InformationCircleIcon className="h-4 w-4" />
                            </div>
                            <div className="absolute bottom-full right-0 mb-2 w-48 p-2 bg-black/90 text-[10px] leading-tight text-slate-200 rounded-lg opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-50 shadow-xl border border-white/10 backdrop-blur-sm">
                                {tooltip}
                            </div>
                        </div>
                    )}
...
const DASHBOARD_TEMPLATES = [
    { 
        ...
        description: 'Complete visibility with all metrics.',
        widgets: ['kpi', 'charts', 'alerts'],
        icon: <SparkleIcon className="h-5 w-5" />
    },
    { 
        id: 'sales', 
        name: 'Sales Focus', 
        description: 'Revenue trends and performance charts.',
        widgets: ['kpi', 'charts'],
    ...
    { 
        ...
        description: 'Real-time alerts and key stats.',
        widgets: ['kpi', 'alerts'],
    ...
    { 
        id: 'minimal', 
        name: 'Minimalist', 
        description: 'Clean view with only core KPIs.',
        widgets: ['kpi'],
...
export const Dashboard: React.FC<{
  ...
  const kpis = useMemo(() => {
    // Total Revenue: Only Completed (Recognized)
    const realizedEvents = eventFinancials.filter(e => e.status === 'Completed');
    const totalRevenue = realizedEvents.reduce((sum, e) => sum + e.revenue, 0);

    // Pipeline: Draft + Quote Sent
    ...
    // Weighted Average Margin: Completed + Confirmed
    // This represents business health more accurately than simple average of per-event margins
    const secureEvents = eventFinancials.filter(e => ['Completed', 'Confirmed'].includes(e.status));
    const secureRevenue = secureEvents.reduce((sum, e) => sum + e.revenue, 0);
    const secureProfit = secureEvents.reduce((sum, e) => sum + e.profit, 0);
    const averageMargin = secureRevenue > 0 ? (secureProfit / secureRevenue) * 100 : 0;
    
    // Upcoming: Future dates, active status
  ...
  const handleApplyTemplate = (widgets: string[]) => {
      if (onUpdateSettings) {
          onUpdateSettings({ userPreferences: { ...appSettings.userPreferences, dashboardWidgets: widgets } as any });
      }
  };
  ...
  return (
    <div className="relative p-4 sm:p-8 space-y-10 pb-20 min-h-screen">
      {/* Header Area */}
      ...
      {activeWidgets.includes('kpi') && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-in-item" style={{ animationDelay: '150ms' }}>
            <ExpandableInsightCard 
                ...
                tooltip="Total recognized revenue from completed events."
                aiContextData={{ 
            ...
            <ExpandableInsightCard 
                ...
                tooltip="Potential revenue from events in Draft or Quote Sent status."
                aiContextData={{
            ...
            <ExpandableInsightCard 
                ...
                note={isUserView ? "Based on 15% rate" : "Booked & Completed"}
                reportType="Profitability Analysis"
                tooltip={isUserView ? "Estimated commission percentage based on current margins." : "Weighted average profit margin for all Confirmed and Completed events."}
                aiContextData={{
            ...
            <ExpandableInsightCard 
                ...
                tooltip="Count of events scheduled for future dates (excluding Canceled)."
                aiContextData={{
      ...
      {isCustomizeOpen && (
        <Modal title="Customize Dashboard" onClose={() => setIsCustomizeOpen(false)}>
            <div className="space-y-6">
                ...
                <div>
                    <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--text-secondary-color)] mb-3">Quick Layout Templates</h4>
                    <div className="grid grid-cols-2 gap-3">
                        {DASHBOARD_TEMPLATES.map(template => (
                            <button
                                key={template.id}
                                onClick={() => handleApplyTemplate(template.widgets)}
                                className="flex items-start gap-3 p-3 rounded-xl border border-white/5 bg-black/20 hover:bg-[var(--primary-accent-color)]/20 hover:border-[var(--primary-accent-color)]/50 transition-all text-left group"
                            >
                                <div className="p-2 rounded-lg bg-white/5 group-hover:bg-[var(--primary-accent-color)]/30 transition-colors">
                                    {React.cloneElement(template.icon as React.ReactElement, { className: 'h-5 w-5 text-slate-300 group-hover:text-white' })}
                                </div>
                                <div>
                                    <span className="block text-sm font-bold text-white group-hover:text-[var(--primary-accent-color)]">{template.name}</span>
                                    <span className="text-[10px] text-slate-400 group-hover:text-slate-300 leading-tight">{template.description}</span>
                                </div>
                            </button>
                        ))}
                    </div>
                ...
                {/* Manual Selection */}
                <div>
                    <p className="text-sm text-[var(--text-secondary-color)] mb-3">Or manually select widgets:</p>
                    {[
                        ...
                        <label key={widget.id} className="flex items-center gap-3 p-3 rounded-lg bg-black/20 border border-white/5 cursor-pointer hover:bg-black/30 mb-2">
                            <input 
                                type="checkbox" 
