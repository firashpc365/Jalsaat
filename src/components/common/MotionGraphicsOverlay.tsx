...
interface MotionGraphicsOverlayProps {
    type?: 'particle-flow' | 'light-flares' | 'abstract-lines' | 'golden-sparkles' | 'liquid';
    color?: string;
    opacity?: number;
...
export const MotionGraphicsOverlay: React.FC<MotionGraphicsOverlayProps> = ({ type, color, opacity, config }) => {
    ...
    const settings = useMemo(() => ({
        ...
        count: config?.particleCount ?? (type === 'liquid' ? 6 : 60),
        speed: config?.particleSpeed ?? (type === 'liquid' ? 0.2 : 0.5),
        ...
        enableOverlap: config?.enableOverlap ?? (type === 'liquid'),
    ...
    useEffect(() => {
            ...
            if (settings.style === 'liquid') {
                ...
                for (let i = 0; i < blobCount; i++) {
                    particles.push({
                        ...
                        radius: Math.min(canvas.width, canvas.height) * (0.3 + Math.random() * 0.3),
                        color: i % 2 === 0 ? settings.baseColor : '#4f46e5', // Alternate between base and a deep blue/purple
                        alpha: 0.6
                    });
                }
            } else {
        ...
        const drawLiquid = () => {
            if (!ctx) return;
            // For liquid effect, we don't clear perfectly to create trails, or we use a specific blend mode
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            
            // Use lighter composite for glowing blend
            ctx.globalCompositeOperation = 'screen';
            // Add a heavy blur filter to the context for the "liquid" meld look
            // Note: filter property might affect performance on large screens
            ...
            // Reset filter for other potential renders
            ctx.filter = 'none';
            ctx.globalCompositeOperation = 'source-over';
            
            animationFrameRef.current = requestAnimationFrame(drawLiquid);
        };
    ...
    return (
        <canvas
            ...
            style={{
                ...
                mixBlendMode: settings.style === 'liquid' ? 'hard-light' : 'normal'
            }}
        />
    );
};
