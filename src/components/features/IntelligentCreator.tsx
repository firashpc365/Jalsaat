...
import { PlusIcon, TrashIcon, DocumentTextIcon, SparkleIcon, NetworkIcon, EditIcon } from '../common/icons';
...
export const IntelligentCreator: React.FC<IntelligentCreatorProps> = ({ services, clients, onClose, onConfirm, setError, mode = 'event' }) => {
  ...
  const handleUpdateParsedItem = (updatedItem: CostTrackerItem) => {
      setParsedData(prev => {
          if (!prev || !prev.cost_tracker) return prev;
          const newTracker = [...prev.cost_tracker];
          if (editingItemIndex !== null && editingItemIndex >= 0 && editingItemIndex < newTracker.length) {
              newTracker[editingItemIndex] = updatedItem;
          }
          return { ...prev, cost_tracker: newTracker };
      });
    ...
    <div className="space-y-4">
        ...
        <div>
            <InputField 
                ...
                onChange={e => setParsedData(p => ({...p, clientName: e.target.value}))} 
                list="creator-client-suggestions"
                autoComplete="off"
            />
            <datalist id="creator-client-suggestions">
                {clients.map(c => (
        ...
        <div>
            ...
            {parsedData?.cost_tracker && parsedData.cost_tracker.length > 0 ? (
                <ul className="mt-1 border rounded-lg p-2 max-h-48 overflow-y-auto space-y-1 custom-scrollbar" style={{ borderColor: 'var(--border-color)', backgroundColor: 'rgba(0,0,0,0.1)' }}>
                    {parsedData.cost_tracker.map((item, index) => (
                        <li key={index} className="text-sm py-1 flex justify-between items-center p-2 rounded" style={{ backgroundColor: 'rgba(0,0,0,0.2)' }}>
                            <div>
                                ...
                                {(item.client_price_sar > 0) && (
                                    <span className="ml-2 text-green-400 font-mono text-xs">SAR {item.client_price_sar.toLocaleString()}</span>
                                )}
                            </div>
                            <div className="flex gap-1">
                                <button onClick={() => setEditingItemIndex(index)} className="text-blue-400 hover:text-blue-300 p-1 bg-white/5 rounded hover:bg-white/10 transition-colors" title="Edit Item"><EditIcon className="h-4 w-4"/></button>
                                <button onClick={() => handleRemoveItem(index)} className="text-red-500 hover:text-red-400 p-1 bg-white/5 rounded hover:bg-white/10 transition-colors" title="Remove Item"><TrashIcon className="h-4 w-4"/></button>
                            </div>
                        </li>
                    ))}
                </ul>
  ...
  return (
    <>
        ...
        {editingItemIndex !== null && parsedData && parsedData.cost_tracker && (
            <PromptItemEditor 
                item={parsedData.cost_tracker[editingItemIndex]}
                event={parsedData as EventItem}
                onClose={() => setEditingItemIndex(null)}
                setError={setError}
                onUpdateItem={handleUpdateParsedItem}
                onLogInteraction={(data) => console.log('Prompt Editor Log:', data)}
            />
        )}
    </>
  );
