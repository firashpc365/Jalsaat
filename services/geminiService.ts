...
const getApiKey = () => {
    // Standard Vite way for browser compatibility
...
const safeGenerateContent = async (params: any): Promise<any> => {
    try {
        ...
        } else if (contents.inlineData || contents.text) {
            contents = [{ role: 'user', parts: [contents] }];
        }
        ...
        if (error.status === 429 || error.code === 429) {
            console.warn("API Quota Exceeded. Automatic fallback to descriptive results.");
            const isJson = params.config?.responseMimeType === 'application/json' ||
                (params.config?.systemInstruction && params.config.systemInstruction.includes('JSON'));
            const mockText = isJson
                ? JSON.stringify({ error: "QUOTA_EXCEEDED", message: "Fallback data active", result: "Sample AI Result" })
                : " [AI QUOTA EXCEEDED] This is a representative response generated by the local fallback system.";
...
export const generateImageForPrompt = async (prompt: string): Promise<string> => {
    ...
    const response = await ai.models.generateImages({
        model: 'imagen-3', // Use standard Imagen 3
        prompt: prompt,
...
export const createEventFromInput = async (input: { text: string, files: File[] }, services: ServiceItem[], useDeepThinking: boolean) => {
    ...
    const response = await safeGenerateContent({
        model: "gemini-1.5-flash", // Use Flash for better availability
        contents: { parts },
        config
    });
...
export const generateVideoPreview = async (prompt: string): Promise<string> => {
    ...
    let operation = await ai.models.generateVideos({
        model: 'veo-1.0-generate-preview', // Use a more standard/placeholder name if Veo 3.1 isn't available
        prompt: prompt,
