...
export const EventDetail: React.FC<EventDetailProps> = ({
    ...
    const handleSaveDetails = () => {
        // Auto-fill contact if selecting existing client
        let updatedEvent = { ...editedEvent };
        const existingClient = clients.find(c => c.companyName === editedEvent.clientName);
        if (existingClient && (!editedEvent.clientContact || editedEvent.clientContact === 'TBD')) {
            updatedEvent.clientContact = existingClient.primaryContactName;
        }

        onUpdateEvent(event.eventId, updatedEvent);
                        ...
                        <ul className="space-y-2 overflow-y-auto custom-scrollbar flex-grow pr-1 max-h-[300px]">
                            {sortedTasks.map(task => {
                                ...
                                return (
                                    <li
                                        key={task.id}
                                        className={`flex items-start gap-3 p-3 rounded-lg border transition-all duration-500 group ${status === 'completed' ? 'bg-green-500/5 border-green-500/20' :
                                    ...
                                    >
                                        <div className="pt-0.5 flex-shrink-0 relative flex items-center justify-center w-5 h-5">
                                            <input
                                                ...
                                                className={`appearance-none w-5 h-5 border-2 rounded-md cursor-pointer transition-all duration-300 ease-out focus:outline-none ${task.isCompleted
                                            ...
                                            />
                                            <svg
                                                className={`absolute w-3.5 h-3.5 text-white pointer-events-none transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${task.isCompleted ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-50 -rotate-90'
                                                    }`}
                                                fill="none"
                                                viewBox="0 0 24 24"
                                                stroke="currentColor"
                                        ...
                                        <div className="flex-grow min-w-0">
                                            ...
                                            {task.dueDate && (
                                                <div className={`text-[10px] mt-1.5 flex items-center gap-1.5 font-medium transition-opacity duration-300 ${task.isCompleted ? 'opacity-50' : 'opacity-100'} ${status === 'overdue' ? 'text-red-400' :
                ...
                <Modal title="Edit Event Details" onClose={() => setIsEditingDetails(false)} onSave={handleSaveDetails}>
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary-color)' }}>Client Name</label>
                                <input
                                    list="client-list"
                                    type="text"
