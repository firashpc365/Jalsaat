...
import { MenuIcon, DownloadIcon, PlusIcon, SparkleIcon, BoltIcon, EyeIcon, HeartIcon, BriefcaseIcon, CakeIcon, UsersIcon, StarIcon, CalendarIcon, Squares2X2Icon } from './common/icons';
...
export const EventList: React.FC<EventListProps> = ({ events, user, users, setView, setSelectedEventId, onAddEvent, setError, setSuccess, onUpdateEvent, isAddEventModalOpen, onToggleAddEventModal, onMenuClick, appSettings, onUpdateSettings }) => {
  ...
  }, [sortedEvents.length, viewOptions, viewMode]);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8 h-full flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
             ...
             <div className="flex bg-black/20 p-1 rounded-lg border border-white/5">
                <button 
                    ...
                    className={`p-2 rounded-md transition-all flex items-center gap-2 text-xs font-bold uppercase ${viewMode === 'grid' ? 'bg-[var(--primary-accent-color)] text-white shadow-sm' : 'text-[var(--text-secondary-color)] hover:text-white'}`}
                    title="Grid View"
                >
                ...
                <button 
                    ...
                    className={`p-2 rounded-md transition-all flex items-center gap-2 text-xs font-bold uppercase ${viewMode === 'calendar' ? 'bg-[var(--primary-accent-color)] text-white shadow-sm' : 'text-[var(--text-secondary-color)] hover:text-white'}`}
                    title="Calendar View"
                >
      ...
      >
        {viewMode === 'calendar' ? (
            <CalendarView events={sortedEvents} onEventClick={handleCardClick} />
        ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {events.length === 0 && (
                    <div className="col-span-full h-full flex flex-col items-center justify-center opacity-50 py-20">
